...
func main() {
	...
	if config.JSON {
		go func() {
			defer writeWg.Done()
			output.WriteURLsJSON(out, results, config.Whitelist, config.RemoveParameters)
		}()
	} else {
		go func() {
			defer writeWg.Done()
			if err = output.WriteURLs(out, results, config.Whitelist, config.RemoveParameters); err != nil {
